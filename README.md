...
- **18 March 2025**: We have updated our ASAP7 tescases, SA implementation for faster and better results and generated macro placement solutions using the latest Circuit Training (dubbed as AlphaChip). The details are as follows.
  ...
  - We train Google’s AlphaChip from scratch, and fine-tune AlphaChip (from the pre-trained checkpoint released in September 2024), for all our testcases. A revised experimental protocol  for both training from scratch and fine-tuning – affords more opportunity for CT to converge: we increase the number of iterations from 200 to 400, and make multiple trials before declaring non-convergence. We provide the tensorboard links for the pre-training, training from scratch and fine-tuning of AlphaChip [here](./Docs/OurProgress/README.md#03182025).
