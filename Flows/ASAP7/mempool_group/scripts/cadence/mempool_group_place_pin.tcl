...
# North
place_tcdm_top_bus tcdm_master_req            out 1 $master_req_bus_length  [expr 0.30*$group_width] [expr 0.51*$group_width] [list M3 M3]
place_tcdm_top_bus tcdm_slave_req             in  1 $slave_req_bus_length   [expr 0.30*$group_width] [expr 0.51*$group_width] [list M5 M3]
place_tcdm_top_bus tcdm_slave_resp            out 1 $slave_resp_bus_length  [expr 0.51*$group_width] [expr 0.67*$group_width] [list M5 M3]
place_tcdm_top_bus tcdm_master_resp           in  1 $master_resp_bus_length [expr 0.51*$group_width] [expr 0.67*$group_width] [list M3 M5]

# Northeast
place_tcdm_top_bus tcdm_slave_req             in  2 $slave_req_bus_length   [expr 0.67*$group_width] [expr 0.80*$group_width] [list M3 M5]
place_tcdm_top_bus tcdm_master_resp           in  2 $master_resp_bus_length [expr 0.67*$group_width] [expr 0.80*$group_width] [list M5 M3]
...
editPin -pin $ports -edge 1 -start $pt1 -end $pt2 -fixedPin -layer [list M3 M5] -spreadDirection clockwise -pattern fill_optimised

# Northeast
place_tcdm_left_bus tcdm_master_req           out 2 $master_req_bus_length [expr 0.15*$group_height] [expr 0.30*$group_height] [list M4 M6]
place_tcdm_left_bus tcdm_slave_resp           out 2 $slave_resp_bus_length [expr 0.15*$group_height] [expr 0.30*$group_height] [list M6 M8]
# East
place_tcdm_left_bus tcdm_master_req           out 0 $master_req_bus_length  [expr 0.30*$group_height] [expr 0.55*$group_height] [list M4 M8]
place_tcdm_left_bus tcdm_slave_req            in  0 $slave_req_bus_length   [expr 0.30*$group_height] [expr 0.55*$group_height] [list M4 M6]
place_tcdm_left_bus tcdm_master_resp          in  0 $master_resp_bus_length [expr 0.55*$group_height] [expr 0.72*$group_height] [list M4 M6]
place_tcdm_left_bus tcdm_slave_resp           out 0 $slave_resp_bus_length  [expr 0.55*$group_height] [expr 0.72*$group_height] [list M6 M8]

set ports [dbget top.terms.name  *dma_req*]
set x [dbget top.fplan.box_urx]
set y1 [expr 0.72*$group_height]
set y2 [expr 0.85*$group_height]
set pt1 [list $x $y1]
set pt2 [list $x $y2]
editPin -pin $ports -edge 2 -start $pt1 -end $pt2 -fixedPin -layer [list M4 M6] -spreadDirection counterclockwise -pattern fill_optimised
...
set pt1 [list $x $y1]
set pt2 [list $x $y2]
editPin -pin $ports -edge 0 -start $pt1 -end $pt2 -fixedPin -layer [list M4 M6] -spreadDirection clockwise -pattern fill_optimised
